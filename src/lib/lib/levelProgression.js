 ...
 *
 * 1. English visibility (the "fade-out" behavior), per latest direction:
 *    - beginner & intermediate: rolling-accuracy-driven by default (last
 *      10 answers, 80% threshold). A manual toggle is available on both
 *      levels and overrides rolling accuracy — when ON, English is
 ...
 *      decides visibility) until the user turns the toggle back OFF.
 *    - advanced: always Arabic-only, no toggle shown (there is no English
 *      to hide on the prompt/answer side at this level by design).
 *    - IMPORTANT: this toggle only ever affects the PROMPT and ANSWER
 *      text. The explanation block (shown after answering) is always
 *      bilingual on every level, regardless of toggle state or rolling
 *      accuracy — explanations are not part of what this function
 *      controls; see Quiz.jsx, which renders explanations unconditionally
 *      bilingual.
 *
 ...
 * final "should English show right now" decision for the PROMPT/ANSWER
 * text only (never the explanation block — see file header).
 *
...
export function shouldShowEnglish(level, manualHideEnglish, answerHistory) {
  if (level === "advanced") return false; // always Arabic-only, no toggle
